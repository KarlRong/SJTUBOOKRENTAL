...
<html>
	...
	<body >
		...
		<h1 style="text-align: center;">本人借出图书</h1><br>
		<div  align="center">
		...
		<table style="background-color:#99bbbb;" border="1" align="center">
			<tr>
				<td colspan="5" style="background-color:#99bbbb;">
					<h2 style="text-align: center;">本人借出图书列表</h2>
				</td>
			</tr>
			...
			<tr valign="center">
				...
				<td style="background-color:#EEEEEE;height:20px;width:200px;text-align:center;"><b>借书用户</b></td> 
...
<h2>书籍 《{$row["bookname"]}》 已借出</h2>
</pre>
				<a href="./book_myloan.php">继续</a></center>
			    </td> 
                </tr>
HTML;
         }
         
        $sql ="UPDATE `book` SET `state`= 0
...
HTML;
        ...
        $sql ="UPDATE `book` SET `borrowerid`= NULL,`state`= 1 
        WHERE `id` = {$_POST["bookid"]} ";
        if($conn->query($sql) == TRUE){
        }
        else{
...
//    echo "数据库连接成功" . "<br>";
    ...
    while($row = $result->fetch_assoc() ) {
        ...
        echo	<<< HTML
			    ...
			    {$row["bookname"]}</td> 
			    <td style="background-color:#EEEEEE;height:100px;width:200px;text-align:center;"> <input type="text" id="field1" value="{$row["borrowerid"]}" style="border:none;height:30px;width:130px;font-size:20px;background-color:#EEEEEE;"><br></td>

HTML;
        if( $row["state"] == 1 ){
            echo	<<< HTML
                 <td style="background-color:#EEEEEE;height:100px;width:200px;text-align:center;"> <input type="text" id="field1" value="未借出" style="border:none;height:30px;width:100px;font-size:20px;background-color:#EEEEEE;"><br></td> 
				...
				<input type="submit" name="button" style="height:30px;width:100px;text-align:center;font-size:20px;" value="同意借出"></button> </td>
				</form>
				</tr> 
HTML;
        } else {
            echo	<<< HTML
				...
				<form method="post" action="{$_SERVER["PHP_SELF"]}"> 
				<input type="hidden" name="bookid"  value="{$row["id"]}">
				<input type="submit" name="button" style="height:30px;width:100px;text-align:center;font-size:20px;" value="上架"></button> </td>
				</form>
				</tr> 
HTML;
        }
